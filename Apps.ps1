...
$Devices = Invoke-GraphAPIRequest -GraphURL 'https://graph.microsoft.com/beta/deviceManagement/managedDevices' -Method 'GET' -AccessToken $token

foreach ($ManagedDevice in $Devices) {
   $AppResults.Add((Invoke-GraphAPIRequest -GraphURL "https://graph.microsoft.com/beta/deviceManagement/managedDevices/$($ManagedDevice.ID)/detectedapps" -Method 'GET' -AccessToken $token))
...
$AllResults | Export-Csv -Path Apps.csv
